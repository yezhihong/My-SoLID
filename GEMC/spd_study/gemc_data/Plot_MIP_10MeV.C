{
	...
	TFile *output = new TFile("SPD_EDep_10MeV.root","recreate");

	TCanvas *c1 = new TCanvas("c1","c1",1200,800);
	TFile *fE1 = new TFile("electron/5mm/SIDIS_SPD_E10MeV_out.root");
	TFile *fE2 = new TFile("electron/1cm/SIDIS_SPD_E10MeV_out.root");
	TFile *fE3 = new TFile("electron/2cm/SIDIS_SPD_E10MeV_out.root");
//	TFile *fE4 = new TFile("electron/5cm/SIDIS_SPD_E10MeV_out.root");
...
*/
	...
	TH2F *hEDep5mm = new TH2F("hEDep5mm","5mm SPD with Electron (0-10 MeV)",100,1e-9, 10,100, 1e-1, 20.0);
    T->Draw("Edep:Ee>>hEDep5mm","","colz");
	c2->cd(2); fE2->cd();gPad->SetLogy(1);
	TH2F *hEDep1cm = new TH2F("hEDep1cm","1cm SPD with Electron (0-10 MeV)",100,1e-9, 10,100, 1e-1, 20.0);
    T->Draw("Edep:Ee>>hEDep1cm","","colz");
	c2->cd(3); fE3->cd();gPad->SetLogy(1);
	TH2F *hEDep2cm = new TH2F("hEDep2cm","2cm SPD with Electron (0-10 MeV)",100,1e-9, 10,100, 1e-1, 20.0);
    T->Draw("Edep:Ee>>hEDep2cm","","colz");
//	c2->cd(4); fE4->cd();gPad->SetLogy(1);
//	TH2F *hEDep5cm = new TH2F("hEDep5cm","50mm SPD with Electron (0-10 MeV)",100,1e-9, 10,100, 1e-1, 20.0);
  //  T->Draw("Edep:Ee>>hEDep5cm","","colz");

	c2->cd(4); fG3->cd();gPad->SetLogy(1);
	TH2F *hGDep2cm = new TH2F("hGDep2cm","2cm SPD with Photon (0-10 MeV)",100,1e-9, 10,100, 1e-1, 20.0);
   ...
   c2->Print("SPD_EDep_2D_10MeV.png");
   c3->Print("SPD_EDep_1D_10MeV.png");

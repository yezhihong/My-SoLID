{
	...
	TFile *output = new TFile("SPD_EDep.root","recreate");

	TCanvas *c1 = new TCanvas("c1","c1",1200,800);
	c1->Divide(1,2);
	TFile *fE1 = new TFile("electron/5mm/SIDIS_SPD_E_out.root");
	TFile *fE2 = new TFile("electron/1cm/SIDIS_SPD_E_out.root");
	TFile *fE3 = new TFile("electron/2cm/SIDIS_SPD_E_out.root");
//	TFile *fE4 = new TFile("electron/5cm/SIDIS_SPD_E_out.root");

 	TFile *fG1 = new TFile("photon/5mm/SIDIS_SPD_G_out.root");
	TFile *fG2 = new TFile("photon/1cm/SIDIS_SPD_G_out.root");
	TFile *fG3 = new TFile("photon/2cm/SIDIS_SPD_G_out.root");
//	TFile *fG4 = new TFile("photon/5cm/SIDIS_SPD_G_out.root");
	...
	fG3->cd(); hG3->Scale(1/10.);hG3->Draw("same");
/*
	c1->cd(1); fE4->cd();
 ...
 */
	...
	TH2F *hEDep5mm = new TH2F("hEDep5mm","5mm SPD with Electron (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Ee>>hEDep5mm","","colz");
	c2->cd(2); fE2->cd();gPad->SetLogy(1);
	TH2F *hEDep1cm = new TH2F("hEDep1cm","1cm SPD with Electron (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Ee>>hEDep1cm","","colz");
	c2->cd(3); fE3->cd();gPad->SetLogy(1);
	TH2F *hEDep2cm = new TH2F("hEDep2cm","2cm SPD with Electron (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Ee>>hEDep2cm","","colz");
//	c2->cd(4); fE4->cd();gPad->SetLogy(1);
//	TH2F *hEDep5cm = new TH2F("hEDep5cm","50mm SPD with Electron (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
//    T->Draw("Edep:Ee>>hEDep5cm","PID==11||PID=-11","colz");

	c2->cd(4); fG1->cd();gPad->SetLogy(1);
	TH2F *hGDep5mm = new TH2F("hGDep5mm","5mm SPD with Photon (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Eg>>hGDep5mm","","colz");
	c2->cd(5); fG2->cd();gPad->SetLogy(1);
	TH2F *hGDep1cm = new TH2F("hGDep1cm","1cm SPD with Photon (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Eg>>hGDep1cm","","colz");
	c2->cd(6); fG3->cd();gPad->SetLogy(1);
	TH2F *hGDep2cm = new TH2F("hGDep2cm","2cm SPD with Photon (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);
    T->Draw("Edep:Eg>>hGDep2cm","","colz");
//	c2->cd(8); fG4->cd();gPad->SetLogy(1);
//	TH2F *hGDep5cm = new TH2F("hGDep5cm","50mm SPD with Photon (0-11 GeV)",110,1e-9, 11000,110, 1e-2, 20.0);

...
/*void FASPD_Segment(string input_filename){{{*/
void FASPD_Segment(string input_filename){
//int main(){
	...
	const int SPD_VP_ID = 5110000; //Front virtual plane
	...
	const int Module = 60; //60 module around the circle
	...
	TString output_filename = Detector_Name +"_"+pid_temp+Form("_%dns.root", (int)(Time_Window/nSec));
	...
	cerr<<"==================================================================================="<<endl; 
	cerr<<Form("--- Overal background events within %d ns time-window for the entire SPD plane", (int)(Time_Window/nSec))<<endl;
	cerr<<Form("     electron = %d,  photon = %d",Count_ele, Count_pho)<<endl;  
	cerr<<"==================================================================================="<<endl; 
	/*}}}*/
	
	const double R_Cut [5] = {96, 120, 146, 175, 210};
		...
		if(1){
			...
			for (int j=0;j<nhit_flux;j++) {
				...
				for (int k=0;k<nhit_flux;k++) {
					...
					//Remove particle that go forward 
					if((int)(flux_tid->at(k))==TID_flux||(int)(flux_mtid->at(k))==TID_flux) continue;
					if((int)(flux_pid->at(k))==PID_flux||(int)(flux_mpid->at(k))==PID_flux) continue;
					...
					if(r_temp > R_Max_In||r_temp<R_Min_In) continue;//Must be within the radius range

					int ID_back = (int) (flux_id->at(k));
					int PID_back = (int) (flux_pid->at(k));
					int TID_back = (int) (flux_tid->at(k));
					int MTID_back = (int) (flux_mtid->at(k));
					bool bSkip = kTRUE; 
					//See whether these backward particles are from the virtual plane on the back
					for (int l=0;l<nhit_flux;l++) {
						if((int)(flux_id->at(l)!=VP_Out)) continue;

						if((int)(flux_tid->at(l))!=TID_back&&(int)(flux_mtid->at(l))!=TID_back) continue;
						if((int)(flux_pid->at(l))!=PID_back&&(int)(flux_mpid->at(l))!=PID_back) continue;

						if((flux_pz->at(l))<-1e-10) continue;
						double x_back = flux_avg_x->at(k)*MM2CM, y_back = flux_avg_y->at(k)*MM2CM;
						double r_back=sqrt(pow(x_back,2)+pow(y_back,2));//cm
						if(r_back > R_Max_In||r_back<R_Min_In) continue;//Must be within the radius range
				...
				double EDep_Sum_ele = 0.0;//that is for the entire SPD plane	
...
*/
				...
				double EDep_Sum_pho = 0.0;//that is for the entire SPD plane
...
int main(){
	...
	FASPD_Segment(input_filename.c_str());

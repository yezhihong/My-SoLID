...
void GetHisto_EM(){
	...
	TChain *T[N];
	T[0]= new TChain("T");
	T[0]->Add("../FASPD_background_SIDIS_He3_EM_mpid.root");
	...
	/*R_flux{{{*/
	TH1F *hr_ele = new TH1F("hr_ele","e+/- radius on SPD", 115,95.,210.);
	hr_ele->SetXTitle("r_{flux} (cm)");
	hr_ele->GetXaxis()->CenterTitle(1);	
	hr_ele->GetXaxis()->SetTitleSize(0.06);

	TH1F *hr_pho = new TH1F("hr_pho","photon radius on SPD", 115,95.,210.);
	...
	/*E_flux log{{{*/
	TH1F *hELog_ele = new TH1F("hELog_ele","e+/- energy (log) on SPD", 100,-2,2.1);
	hELog_ele->SetXTitle("E_{flux} (MeV) (log)");
	hELog_ele->GetXaxis()->CenterTitle(1);	
	hELog_ele->GetXaxis()->SetTitleSize(0.06);

	TH1F *hELog_pho = new TH1F("hELog_pho","photon energy (log) on SPD", 100,-2,2.1);
	...
	/*E_flux log vs r{{{*/
	TH2F *hRELog_ele = new TH2F("hRELog_ele","e+/- energy (log) on SPD", 115, 95.,210., 100,-2,2.1);
	...
	TH2F *hRELog_pho = new TH2F("hRELog_pho","photon energy (log) on SPD",115,95., 210, 100,-2,2.1);
	...
	/*E_flux{{{*/
	TH1F *hE_ele = new TH1F("hE_ele","e+/- energy on SPD", 100,0,1e4);
	hE_ele->SetXTitle("E_{flux} (MeV)");
	hE_ele->GetXaxis()->CenterTitle(1);	
	hE_ele->GetXaxis()->SetTitleSize(0.06);

	TH1F *hE_pho = new TH1F("hE_pho","photon energy on SPD", 100,0,1e4);
	...
	/*EDep_flux (log){{{*/
	TH1F *hEdepLog_ele = new TH1F("hEdepLog_ele","e+/- energy-deposit (log) on SPD", 100,-8,1.0);
	hEdepLog_ele->SetXTitle("Edep_{flux} (log) (MeV)");
	hEdepLog_ele->GetXaxis()->CenterTitle(1);	
	hEdepLog_ele->GetXaxis()->SetTitleSize(0.06);

	TH1F *hEdepLog_pho = new TH1F("hEdepLog_pho","photon energy-deposit (log) on SPD", 100,-8,1.0);
	...
	TH2F *hELogEdepLog_ele = new TH2F("hELogEdepLog_ele","e+/- energy-deposit (log) vs E(log) on SPD", 100,-2,2.1,100,-8,1.0);
	...
	TH2F *hELogEdepLog_pho = new TH2F("hELogEdepLog_pho","photon energy-deposit (log) vs E(log) on SPD", 100,-2,2.1,100,-8,1.0);
	...
	/*EDep_flux (log) vs r{{{*/
	TH2F *hREdepLog_ele = new TH2F("hREdepLog_ele","e+/- energy-deposit (log) vs r on SPD", 115,95.,210.,100,-8,1.0);
	...
	TH2F *hREdepLog_pho = new TH2F("hREdepLog_pho","photon energy-deposit (log) vs r on SPD", 115,95.,210.,100,-8,1.0);
	...
	TH2F *hPhiEdepLog_ele = new TH2F("hPhiEdepLog_ele","e+/e- energy-deposit (log) vs r on SPD",360,0.,360.,100,-8,1.0);
	...
	TH2F *hPhiEdepLog_pho = new TH2F("hPhiEdepLog_pho","photon energy-deposit (log) vs r on SPD",360,0.,360.,100,-8,1.0);
	...
	/*EDep_flux{{{*/
	TH1F *hEdep_ele = new TH1F("hEdep_ele","e+/- energy-deposit on SPD", 100,0,10.0);
	hEdep_ele->SetXTitle("Edep_{flux} (MeV)");
	hEdep_ele->GetXaxis()->CenterTitle(1);	
	hEdep_ele->GetXaxis()->SetTitleSize(0.06);

	TH1F *hEdep_pho = new TH1F("hEdep_pho","photon energy-deposit on SPD", 100,0,10.0);
	...
	double G_EDep_Sum = 0.0;
	double E_EDep_Sum = 0.0;
	double G_E_Sum = 0.0;
	double E_E_Sum = 0.0;
	double G_Rate_Sum = 0.0;
	...
	for(int j=0;j<N;j++){	
		...
		for(int i=0;i<Nevt;i++){
			...
			if(abs(PID_flux)==11){
				E_EDep_Sum += EDep*1000.;
				E_E_Sum    += E*1000.;
			}
			if(abs(PID_flux)==22){
				G_EDep_Sum += EDep*1000.;
				G_E_Sum    += E*1000.;
			}
			if(EDep<1e-21)
				EDep = 1e-10;
			...
			cerr<<"&&&&&Electron: E_Avg = "<<E_E_Sum/E_N_Sum<<"  EDep_Avg = "<<E_EDep_Sum/E_N_Sum<<endl;
			cerr<<"&&&&&  Photon: E_Avg = "<<G_E_Sum/G_N_Sum<<"  EDep_Avg = "<<G_EDep_Sum/G_N_Sum<<endl;
			cerr<<Form("&&&&& E_Rate = %e",E_Rate_Sum)<<Form("  G_Rate = %e",G_Rate_Sum)<<endl;

			TFile *new_file = new TFile("background_histo_EM.root","recreate");

...
TString LASPD(string input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	*/
	...
	const int VP_ID = 5210000; //Front virtual plane
	...
	/*Histogram Define{{{*/
    TH2F *hist_crg = new TH2F("hist_crg","Triggerred Electrons on LASPD",200,-200,200,200,-200,200);
	hist_crg->SetXTitle("x");
	hist_crg->SetYTitle("y");

	TH2F *hist_pho = new TH2F("hist_pho","Triggerred Photon on LASPD",200,-200,200,200,-200,200);
...
void RunAll(TString input_file){
   ...
   while(!(runlist.eof())){
	  ...
	  TString output_file = LASPD(filename.Data());
//	  if(output_file!="0"){
		 // gSystem->Exec(Form("rm -f file_LASPD/%s", output_file.Data()));
		 // gSystem->Exec(Form("mv %s file_LASPD/", output_file.Data()));
//	  }
   }

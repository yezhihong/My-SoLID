...
void GetDISRate(){
	...
	TH2F *hEDep_dis = new TH2F("hEDep_dis","EDep from e^{#pm} (DIS) on LASPD", 80,70,150,100,0,10);
	hEDep_dis->SetXTitle("r_{flux}");	      hEDep_dis->SetYTitle("E_{Dep} (MeV)");
	hEDep_dis->GetXaxis()->SetTitleSize(0.06); hEDep_dis->GetYaxis()->SetTitleSize(0.06);
	hEDep_dis->GetXaxis()->CenterTitle(1);	  hEDep_dis->GetYaxis()->CenterTitle(1);
	...
	for(int i=0;i<Nevt;i++){
		...
		if(abs(PID_flux)==11||PID_flux==22){
			...
			hEDep_dis->Fill(r_flux, EDep*1000., rate*convert);
		}
	}
    ...
    hEDep_dis->Draw("colz");
	c4->cd(2);
	TH1F* hEDep_dis_px = (TH1F*) hEDep_dis->ProfileX("hEDep_dis_px");
    hEDep_dis_px->Draw();

	c1->Print("Rate_Electron_DIS.png");
	c2->Print("Rate_Photon_DIS.png");
	c3->Print("Rate_Both_DIS.png");
	c1->Print("Rate_Electron_DIS.pdf");
	c2->Print("Rate_Photon_DIS.pdf");
	c3->Print("Rate_Both_DIS.pdf");
	c4->Print("Rate_Both_DIS_EDep.pdf");
	c4->Print("Rate_Both_DIS_EDep.png");


   TFile *new_file = new TFile("histo_dis.root","recreate");
   new_file->cd();
   ...
   hxy1->Write(); hxy2->Write(); hr1->Write(); hr2->Write();
	hEDep_dis->Write();hEDep_dis_px->Write();
   new_file->Close();
}

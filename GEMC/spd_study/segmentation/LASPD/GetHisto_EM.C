...
void GetHisto_EM(){
	...
	TChain *T[N];
	T[0]= new TChain("T");
	T[0]->Add("../LASPD_background_SIDIS_He3_EM_mpid.root");
	...
	/*E_flux log vs r{{{*/
	TH2F *hRELog_ele = new TH2F("hRELog_ele","e+/- energy (log) on SPD", 65,75, 140, 100,-2,2.1);
	...
	TH2F *hRELog_pho = new TH2F("hRELog_pho","photon energy (log) on SPD",65,75,140, 100,-2,2.1);
	...
	/*EDep_flux (log) vs r{{{*/
	TH2F *hREdepLog_ele = new TH2F("hREdepLog_ele","e+/- energy-deposit (log) vs r on SPD", 65,75.,140.,100,-8,1.0);
	...
	TH2F *hREdepLog_pho = new TH2F("hREdepLog_pho","photon energy-deposit (log) vs r on SPD", 65,75.,140.,100,-8,1.0);

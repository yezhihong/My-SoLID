...
void GetRate(){
	...
	TH2F *hEDep = new TH2F("hEDep","EDep from e^{#pm} (DIS+#pi^{0}+#pi^{#pm}+p+EM) on LASPD", 80,70,150,100,0,20);
   	hEDep->SetXTitle("r_{flux}");	      hEDep->SetYTitle("E_{Dep} (MeV)");
	hEDep->GetXaxis()->SetTitleSize(0.06); hEDep->GetYaxis()->SetTitleSize(0.06);
	hEDep->GetXaxis()->CenterTitle(1);	  hEDep->GetYaxis()->CenterTitle(1);
	/*}}}*/
	...
	for(int i=0;i<Nevt;i++){
		...
		if(E>=0.0){
			...
			if(abs(PID_flux)==11||PID_flux==22){
				...
				hEDep->Fill(r_flux, EDep*1000., rate*convert);
			}
		}
	}
    ...
    hEDep->Draw("colz");
	c4->cd(2);
	TH1F* hEDep_px = (TH1F*) hEDep->ProfileX("hEDep_px");
    hEDep_px->Draw();

	c1->Print("Rate_Electron.png");
	c2->Print("Rate_Photon.png");
	c3->Print("Rate_Both.png");
	c1->Print("Rate_Electron.pdf");
	c2->Print("Rate_Photon.pdf");
	c3->Print("Rate_Both.pdf");
	c4->Print("Rate_Both_EDep.pdf");
	c4->Print("Rate_Both_EDep.png");

	TFile *new_file = new TFile("histo_all.root","recreate");
	new_file->cd();

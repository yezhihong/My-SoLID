...
void GetHit(string input_filename)
{
	...
	//sprintf(output_filename, "%s_hit.root",the_filename);
...
//	const double SPD_Thick =0.5; //cm
	const double Hit_Min1 = -0.5;
	const double Hit_Max1 = 65.5;
	const double Hit_Min = -0.5;
	const double Hit_Max = 20.5;
	const int Bin = (int)(Hit_Max-Hit_Min);
	const int Bin1 = (int)(Hit_Max1-Hit_Min1);

	TH1F *h2 = new TH1F("h2","Hit Counts in SPD", Bin1, Hit_Min1, Hit_Max1); //X->Hit Count
	h2->SetXTitle("Hit");
	...
	for(Int_t i=0;i<nevent;i++){
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			//	if (*(flux_ID+j)>=5000000){  //SPD && its virtual plane
			if (*(flux_ID+j)==5100000){  //SPD
				//if (*(flux_ID+j)==5110000){  //Virtual Plane
		...
		cerr<<" --- Gamma_2_E = "<< G_2_E  <<" %= "<< G_2_E*1.0/(nevent*1.0)*100.0<<"%"<<endl;
		...
		gPad->SetLogy(1);
		h2->Draw("");
		input_png.ReplaceAll("_zhit_","_hit1D_");
		c2->Print(input_png.Data());

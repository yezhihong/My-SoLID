...
/*
	...
	*/
	...
	double MRPC_Threshold = 1.; //GeV for EC cut, the cut could be tight if using Jin's curves
	...
	if(input_filename.Contains("after")){//after means that spd is before mrpc
		...
		Z_MRPC = 407.0; //cm VP position, 407.5 is the real positon of MRPC
		Z_Out = 409.0; //cm SPD VP position
	}
	...
	for(Int_t i=0;i<nselected;i++){
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			R_EC = abs((Z_EC - Z_MRPC) * tan(vertex_theta/DEG) + r);
			...
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom<MRPC_Threshold){//#Eelectrons going out 
				if(FirstOne0<1)
					Count_Low+=1.0;
				FirstOne0 ++;;
			}
			...
			if((*(flux_pid+j)==Electron)&&*(flux_ID+j)==ID_Pick && fmom>=MRPC_Threshold){//#Eelectrons going out 
				//Count_Em++;
				if(FirstOne2<1)
					Count_Cut_Em+=EC_Cut;
				FirstOne2 ++;;
            ...
            //Count by slides
			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==ID_Pick && fmom>=MRPC_Threshold)//#Photons going in
				MRPC_In_G[Module_ID][Slide_ID]+=EC_Cut;
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom>=MRPC_Threshold){//#Eelectrons going out 
				if(FirstOne3<1)
					MRPC_In_E[Module_ID][Slide_ID]+=EC_Cut;
				FirstOne3 ++;;
			}
		}
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==ID_Pick && fmom>=MRPC_Threshold ){//#Photons going out 
				MRPC_Out_G[Module_ID][Slide_ID]+=EC_Cut;
			}
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom>=MRPC_Threshold){//#Eelectrons going out 
				Count_Both++;
				if(FirstOne4<1)
					MRPC_Out_E[Module_ID][Slide_ID]+=EC_Cut;
				FirstOne4 ++;;
			}

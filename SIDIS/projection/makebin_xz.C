...
int makebin(int target_flag, int particle_flag, int z_flag);

////////////////////////////////////////
//Only bin on x and z to match SBS binning
// -- Z. Ye, 03/05/2015
//
/*int main{{{*/
int main(){
	...
	for(int i=1;i<=8;i++){
				z_flag = i;
				err = makebin(target_flag, particle_flag, z_flag);
		}
		return err;
}
/*}}}*/
...
int makebin(int target_flag, int particle_flag, int z_flag){
	...
	cerr<<Form(" For %f<z<%f Got total number of events = %e",zmin,zmax, (double)(T->GetEntries()))<<endl;
...
//	const double dilute_factor = 0.85; //85% overall efficiency,1/3 for He3 
	...
	prefix = "./xz_databases/";
	filename = Form("%s_%s_%d.dat",target.Data(), particle.Data(),z_flag);
	new_filename = prefix + filename;
	...
	if (particle_flag==1){
		prefix = "./xz_rootfiles_p/";
	}else{ 
		prefix = "./xz_rootfiles_m/";
	}
	filename = Form("%s_%s_%d.root",target.Data(), particle.Data(),z_flag);
	new_filename = prefix + filename;
	...
	if (particle_flag==1){
		prefix = "./xz_databases_p/";
	}else{
		prefix = "./xz_databases_m/";
	}
	new_filename = prefix + filename;
...
/*	int xbin_fix = (int)((X_MAX-X_MIN)/x_step);
	...
	xbin = 7;
	double x1[8] = {0.05,0.15,0.25,0.35,0.45,0.55,0.65,0.75};
	...
	for (Int_t j=0;j<xbin;j++){
		...
		if (target_flag ==1){
			...
			outf_total << 0 << " \t" << j << " \t"
				...
				<< N_out
				<< endl;
			cerr<<Form("---- Working on x=(%d)%f, N=%e",j,h1x->GetMean(),N_out)<<endl;
		}
		/*}}}*/

		histoname.Form("h_%d_%d",0,j);

...
/*void plot_neutron(target_flag,particle_flag,Int_t Q2_flag =1){{{*/
void plot_neutron(Int_t target_flag = 3,Int_t i=1, Int_t j=1){
  ...
  TString filename;
  filename.Form("./pretz_both/%s_%s_%d.dat",target.Data(),particle.Data(),Q2_flag);
  ...
  while(infile >> Q2_temp >> count2){
	...
	if (Astat[ncount]>0.&&Astat[ncount]<0.05){
	  cerr<<Form("---Q2=%d, x = %f, Asys= %f",Q2_flag, x[ncount], Astat[ncount])<<endl;
	  A[ncount]=0.0;
	  Astat[ncount] *= factor*scale; //coef[0]->Sivers,coef[1]->Collins, coef[2]->Pretzelosity
	  ncount ++;
	}
      }
    }
  ...
  if(particle_flag==1){
    ...
    titlename.Form("#scale[0.8]{<Q^{2}>=%2.1f GeV^{2}}", (Q2min+Q2max)*0.5);
...
/*void plot_neutron_new(target_flag,particle_flag,Int_t Q2_flag =1){{{*/
void plot_neutron_new(Int_t target_flag = 3,Int_t i=1, Int_t j=1){
  //flag_t: 0->pretzelocity, 1->collins, 2->sivers
  //flag: 1->ploting Asym axis
  ...
  g2->GetXaxis()->SetTitle("x");
  //g2->GetYaxis()->SetTitle("A_{UT}^{sin(3#phi-#phi_{S})}");
  g2->GetYaxis()->SetTitle("Pretzelocity Asy. (%)");
  g2->GetXaxis()->SetNdivisions(506);
  g2->GetYaxis()->SetNdivisions(506);
  g2->GetXaxis()->CenterTitle(0);
  g2->GetXaxis()->SetTitleFont(22);
  ...
  if(particle_flag==1){
    ...
    titlename.Form("#scale[1.0]{<Q^{2}>=%2.1f GeV^{2}}", (Q2min+Q2max)*0.5);
...
void plot_neutron_sbs(Int_t target_flag = 3,Int_t i=1){
  ...
  double x[10], N[10], dummy;
	TString Com;

	sbs_pip >> Com >> Com >> Com >> Com >> Com >> Com;
	for(int i=0;i<10;i++){
		sbs_pip >> Com >> dummy >> x[i] >> dummy >> N[i] >> dummy;
		cerr<<Form("pi+: #%d  x = %f, N = %f",i, x[i],N[i])<<endl;
	}
	sbs_pip.close();
	for(int i=0;i<5;i++){
		if(fabs(x[i]-x[i+5])<0.001){
			Asys_pip_sbs[i] = 1./sqrt(N[i]+N[i+5]) * 100.;
			x_pip_sbs[i] = x[i];
	...
	for(int i=0;i<10;i++){
		sbs_pim >> Com >> dummy >> x[i] >> dummy >> N[i] >> dummy;
		cerr<<Form("pi-: #%d  x = %f, N = %f",i, x[i],N[i])<<endl;
	}
	sbs_pim.close();
	for(int i=0;i<5;i++){
		if(fabs(x[i]-x[i+5])<0.001){
			Asys_pim_sbs[i] = 1./sqrt(N[i]+N[i+5]) * 100.;
			x_pim_sbs[i] = x[i];
	...
	double Asys_pip_solid[6];
	double Asys_pim_solid[6];
	ifstream solid_pip("3he_pip_xfit.dat");  
	ifstream solid_pim("3he_pim_xfit.dat");  
	int dum;

	solid_pip >> Com >> Com >> Com >> Com >> Com >> Com
	...
	for(int i=0;i<6;i++){
		...
		x_pip_solid[i] += 0.02;	
		Asys_pip_solid[i]*=100.0;
		cerr<<Form("solid-pi+: #%d  Asys = %f",dum, Asys_pip_solid[i])<<endl;
	}
	solid_pip.close();

	solid_pim >> Com >> Com >> Com >> Com >> Com >> Com
		>> Com >> Com >> Com >> Com >> Com >> Com >> Com;
	cerr<<"--- Pim "<< Com <<endl;
	for(int i=0;i<6;i++){
...
gr22->SetMarkerSize(1.5);
  ...
  if(particle_flag==1){
    ...
    titlename.Form("#scale[1.4]{(#pi^{+}, neutron)}");
    t1->DrawLatex(0.25,0.85,titlename);
    titlename.Form("#scale[1.0]{Integral on (Q^{2},z,P_{T})}");
    t1->DrawLatex(0.5,0.20,titlename);
  }
  if(particle_flag==2){
   	  gr2->Draw("*same");
...
void plot_pretz_evolved(Int_t targ, Int_t part){
  ...
  while(!infile3.eof()){
	  ...
	  ppip_full[i_full] *= scale;
	  ppim_full[i_full] *= scale;
	  npim_full[i_full] *= scale;
	  npip_full[i_full] *= scale;
...
void pretz(){
  ...
  c1->SetFillColor(10);
  c1->cd();
  plot_neutron(2,1,2);//(target,particle,Q2)
  ...
  //c2->Divide(1,2);
  c2->cd();
  plot_neutron(2,2,2);//(target,particle,Q2)
  plot_pretz_evolved(2,2);//(target, particle)
  l1->Draw();


  c1->Print("SoLID_neutron_pip_pretz.pdf");
  c1->Print("SoLID_neutron_pip_pretz.png");
  c2->Print("SoLID_neutron_pim_pretz.pdf");
  c2->Print("SoLID_neutron_pim_pretz.png");
}
/*}}}*/
...
//  gPad->SetBottomMargin(0.10);
  plot_neutron_new(2,1,2);//(target,particle,Q2)
 ...
 // c2->SetFillColor(10);
  ...
  //gPad->SetTopMargin(0.005);
  //gPad->SetBottomMargin(0.10);
   plot_neutron_new(2,2,2);//(target,particle,Q2)
   //plot_neutron_sbs(2,2);//(target,particle,Q2)
  plot_pretz_evolved(2,2);//(target, particle)
  l1->Draw();


//  c1->Print("SoLID_neutron_both_pretz.pdf");
//  c1->Print("SoLID_neutron_both_pretz.png");
}
/*}}}*/

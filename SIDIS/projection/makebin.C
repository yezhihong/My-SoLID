...
//	const double dilute_factor = 0.85; //85% overall efficiency,1/3 for He3 
	...
	const double polarization = 0.6 * 0.6 / 2. ; //60% polarization,1/2 here is for separating Asivers,Acollins,Ap..
	const double detector_eff = 0.85*0.85; //85% detector efficiency for electrons and hadrons
	...
	for (Int_t i=0;i<ptbin;i++){
		...
		double single_bin_raw = single_bin/(dilute_factor)/polarization/target_factor/detector_eff;
		...
		total_eve *= dilute_factor*polarization*target_factor*detector_eff;
		...
		while(xb_max<1000){
			...
			while(nevent_x <=single_bin_raw &&xb_max<1000){
				nevent_x += h2->GetBinContent(xb_max++);
			}
			xmax = h2->GetBinCenter(xb_max);

			if(xmax<x1[new_bin-1]+x_step){
				//  cerr<<Form("    Readjust this bin: x=%f, xb=%d",xmax,xb_max)<<endl;
				xmax = x1[new_bin-1]+x_step;
				xb_max =(int)( (xmax-X_MIN)/((X_MAX-X_MIN)/1000.));
			}

			x1[new_bin] = xmax;
				cerr<<Form("    #%d bin: x=%f, xb=%d ", new_bin, xmax, xb_max)<<endl;
		}

		xbin = new_bin;
		/*}}}*/

		//	if (xbin>total_eve/single_bin) 
		...
		for (Int_t j=0;j<xbin;j++){
			...
			cerr<<Form("---- Working on pt=%f, x=%f",h1pt->GetMean(),h1x->GetMean())<<endl;

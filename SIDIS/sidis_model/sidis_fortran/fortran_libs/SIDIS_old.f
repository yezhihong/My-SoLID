...
***********************************************************************
     ...
     c p_phmax,sigma)
      ...
      real OMEGA,Q(3),Q2,QMAG,QMU2
      real QMU2_G,X,S,Y,z,theta_pq,phiq,pt
...
c     final electron mometum and angle
      ...
      Etheta=(e_thmin+e_thmax)/2.*3.1415926/180.
      ephi=0.
...
c     hadron momentum
      ...
      Ptheta=(p_thmin+p_thmax)/2.*3.1415926/180.
      pphi=(p_phmin+p_phmax)/2.*pi/180.
      ...
      IF(rc_vec(4)**2.LT.p2_rc+mn**2+0.001) GO TO 999
...
C
     ...
     c     /SQRT(Q2)/pf_h)
      ...
      call setctq6(iset)
      do k=-5,5               
         pdf(k) = real(Ctq6Pdf(k,dble(x),dble(sqrtq2)))
      enddo

      call bkk(dble(z),dble(qmu2_G),dupi1,dspi1,duk1,ddk1)
      dupi=real(dupi1)
      dspi=real(dspi1)
      duk=real(duk1)
      ddk=real(ddk1)
     ...
     >           +pdf(-3)*dspi+pdf(3)*dspi)/9.0
         ...
         rnorm1=2.0*PI/137.035**2*s*x*(1.0+(1.0-y)**2)/QMU2**2
...
c     ds/domegadedzdpt^2dphi in the unit of b/GeV^3/sr
         ...
         decay_pi=exp(-25.*mpi/(2.6*PF_H*3.0))
...
c     ds/domegadedzdpt^2dphi in the unit of b/GeV^3/sr
     ...
     c        *1E-3
         ...
         decay_k=exp(-25.*mk/(1.24*PF_H*3.0))

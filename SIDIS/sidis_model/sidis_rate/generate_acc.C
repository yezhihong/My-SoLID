...
void generate_acc(TString particle,int hadron_flag){
	//particle: pion    kaon
...
//	accep_had=(TH2F*)accep_ele_forward->Clone("acceptance_hadron");//pion is normal acceptance histogram, the same as electron acceptance
	...
	for(Long64_t i=0;i<N_entries;i++){
		...
		if(theta_ele*DEG>=7.0&&theta_ele*DEG<=30.0&&mom_ele>=1.0&&mom_ele<=10.0&&W>=2.3&&Wp>=1.6&&z>0.3&&z<0.7){//any additional cuts should be added in here
...
/*
	...
	can_forward->cd(6);
	hf_z_pt->Draw("COLZ");
	can_forward->Print(Form("OldACC_NewXS_%s_%s_Forward.pdf", particle.Data(), charge.Data()));
	can_forward->Print(Form("OldACC_NewXS_%s_%s_Forward.png", particle.Data(), charge.Data()));
	...
	can_large->cd(6);
	hl_z_pt->Draw("COLZ");
	can_large->Print(Form("OldACC_NewXS_%s_%s_Large.pdf", particle.Data(), charge.Data()));
	can_large->Print(Form("OldACC_NewXS_%s_%s_Large.png", particle.Data(), charge.Data()));
	...
	can_total->Print(Form("OldACC_NewXS_%s_%s_Total.pdf", particle.Data(), charge.Data()));
	can_total->Print(Form("OldACC_NewXS_%s_%s_Total.png", particle.Data(), charge.Data()));
	...
	c1->cd(4);
	hl_theta->Draw();
	c1->Print(Form("OldACC_NewXS_%s_%s_ThetaP.pdf", particle.Data(), charge.Data()));
	c1->Print(Form("OldACC_NewXS_%s_%s_ThetaP.png", particle.Data(), charge.Data()));
*/
	/*}}}*/
